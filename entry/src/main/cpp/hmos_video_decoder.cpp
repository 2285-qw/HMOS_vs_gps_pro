...
// 辅助宏
#define CHECK_DECODER(decoder) if (!decoder) { LOGE("Decoder is null"); return false; }
#define CHECK_DECODER_PTR(decoder) if (!decoder) { LOGE("Decoder is null"); return; }
...
HmosVideoDecoder::~HmosVideoDecoder() {
    ...
    LOGI("Decoder destroyed, stats: submitted=%llu, decoded=%llu, rendered=%llu",
         framesSubmitted_, framesDecoded_, framesRendered_);
}
...
bool HmosVideoDecoder::initialize(const std::string& mimeType, int32_t width, int32_t height) {
    ...
    LOGI("初始化解码器: %{public}s,%{public}dx%{public}d", 
    ...
    // 检查硬件解码能力
    OH_AVCapability* hwCapability = OH_AVCodec_GetCapabilityByCategory(
        mimeConst, 
        false,  // 解码器
        HARDWARE
    );
    ...
    if (hwCapability != nullptr && OH_AVCapability_IsHardware(hwCapability)) {
        ...
        // 尝试软件解码
        OH_AVCapability* swCapability = OH_AVCodec_GetCapabilityByCategory(
            mimeConst,
            false,
            SOFTWARE
        );
    ...
    initialized_ = true;
    LOGI("解码器初始化成功 (Hardware: %{public}s, LowVersion: %{public}s)", 
         useHardware_ ? "Yes" : "No", isLowVersion_ ? "Yes" : "No");
...
void HmosVideoDecoder::setCallbacks(
    OnFrameDecodedCallback frameCallback,
...
bool HmosVideoDecoder::decodeFrame(const uint8_t* data, size_t size, 
                                  int64_t timestamp, uint32_t flags) {
    OH_LOG_ERROR(LOG_APP, "解码调用decodeFrame");
    
    if (!started_) {
        LOGE("Decoder not started");
        return false;
    }
    
    if (!decoder_) {
        // 模拟模式：直接生成测试图像
        auto frame = std::make_shared<VideoFrame>(width_, height_, width_, timestamp, AV_PIXEL_FORMAT_NV21);
        ...
        if (flags & AVCODEC_BUFFER_FLAGS_EOS) {
            LOGI("Simulated mode: EOS received");
            if (errorCallback_) {
    ...
    LOGD("Queueing frame for decoding: size=%{public}zu, timestamp=%{public}lld, flags=%{public}u", 
    ...
    if (decoder_ /*|| surface_ != nullptr*/) {
        ...
        if (decodedFrames_.empty() && timeoutMs > 0) {
            ...
            if (timeoutMs == -1) {
                ...
                lock.unlock();
                std::unique_lock<std::mutex> outputLock(outputMutex_);
                outputCond_.wait_for(outputLock, std::chrono::milliseconds(timeoutMs), 
                                   [this]() {
                    std::lock_guard<std::mutex> frameLock(frameMutex_);
        ...
        LOGD("Returning decoded frame: %dx%d, timestamp=%lld", 
...
void HmosVideoDecoder::OnInputBufferAvailable(OH_AVCodec* codec, uint32_t index, 
...
void HmosVideoDecoder::OnOutputBufferAvailable(OH_AVCodec* codec, uint32_t index, 
                                              OH_AVBuffer* buffer, void* userData) {
...
bool HmosVideoDecoder::processPendingInputs() {
    ...
    LOGD("解码输入缓冲区已推入: index=%{public}u, pts=%{public}lld, size=%{public}zu, flags=%{public}u", 
         buffer.index, input.timestamp, input.data.size(), input.flags);
    return true;
}
...
VideoFramePtr HmosVideoDecoder::createVideoFrameFromBuffer(uint32_t index, OH_AVBuffer* buffer) {
    ...
    LOGD("解码已创建视频帧: %{public}dx%{public}d, timestamp=%{public}lld, size=%{public}zu", 
...
bool HmosVideoDecoder::fillInputBuffer(uint32_t index, OH_AVBuffer* buffer, PendingInput& input) {
    ...
    if (static_cast<int32_t>(copySize) > bufferSize) {
        LOGW("Data size (%zu) exceeds buffer capacity (%d), truncating", 
             copySize, bufferSize);
        copySize = bufferSize;
    }
